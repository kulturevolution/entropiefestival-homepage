export async function getUpcomingEventId(locale: string): Promise<number> {
  const response = await fetch(
    `${process.env.NEXT_CMS_API}/homepage-settings?populate=upcoming_event&locale=${locale}`,
    {
      headers: {
...
export async function getEventData(id: number, locale: string): Promise<any> {
  const response = await fetch(
  ...
  return {
    ...
    background_portrait:
      result.data.attributes?.background_portrait?.data?.attributes?.url,
  };
}
